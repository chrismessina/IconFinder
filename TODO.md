...
6. UI/UX priorities: Any specific visual direction (sidebar, list/grid toggle, dark mode tweaks, quick preview), and do you want live filtering as you type?
  ...
  UI priorities:
    ...
    - Add ability to copy icon resource and to set copy format in settings (e.g. icns, png, pdf)
...
- [x] Replace deprecated sheet APIs: use modern sheet presentation (block-based) instead of `beginSheetModalForWindow:modalDelegate:didEndSelector:`.
...
- [ ] SwiftUI app target (macOS app lifecycle):
  - [ ] App structure with menu commands (Scan, Stop, Clear Cache, Settings...).
